...
// Unlike a fullscreen click-catching MouseArea, this never swallows the click
// it reacts to, so the click still reaches whatever was really hit — a bar
// icon underneath, for instance. Pair it with the overlay's own `mask:
// Region`, which shrinks the window's input region to its card.
//
...
//
// TODO: unwired. Widgets/PopoutSurface.qml still uses the fullscreen MouseArea
// this replaces, so popouts swallow clicks meant for the bar.
