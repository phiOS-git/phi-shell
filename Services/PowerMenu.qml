...
// unlock IPC at all), and bolting double-tap timing onto that same
// handler would mean every future reader has to re-verify the security-
// critical path still behaves correctly around the new timing logic. This
...
Singleton {
    ...
    // deliberate double press, short enough the menu opening after a
    // single press doesn't feel sluggish.
    ...
    // replaces it (human double-taps are rarely faster than ~100-200ms
    // apart), which reads as intentional rather than a glitch, and avoids
    // a felt delay on the much more common single-press case.
    function _onTrigger() {
        const now = Date.now()
