...
// Thin wrapper over Quickshell.Hyprland — the one file outside Config/ allowed
// to touch this service surface. Every bar module reads this never
// Quickshell.Hyprland directly, so a 0.3.x API break lands in one place. Named
// HyprlandBridge, not Hyprland, so it's never ambiguous with the real
// singleton it wraps. Kept deliberately thin. Per-monitor filtering (which
// workspaces belong to this screen, whether the active window is on this
// screen) is each module's own job, comparing `HyprlandWorkspace.monitor.name`
// / `HyprlandToplevel.monitor.name` against the bar's own `screen.name` both
// real Wayland output names. This file does not cache a HyprlandMonitor
// reference for a screen: Hyprland.monitorFor(screen) is a plain invokable,
// not a NOTIFYing property, so nothing here could keep it current — reading
// `workspaces`/`activeToplevel` reactively and filtering by name avoids
// needing that call at all.

Singleton {
    id: root

    // Hyprland.workspaces is a real QAbstractListModel, sorted by id natively
    // — re-exported as-is, not copied into a `.values` array: a Repeater bound
    // directly to this stable model tracks individual row changes (a
    // workspace's own `active` flipping) without rebuilding every delegate,
    // which a freshly filtered/sorted array would.
    readonly property var workspaces: Hyprland.workspaces

    // The single globally-activated window, or null. Whether it belongs to a
    // given bar's own monitor is that module's own comparison — this property
    // only answers "what is active anywhere".
    ...
    // workspaces/activeToplevel above. Unverified against real hardware if
    // `Hyprland.toplevels` turns out not to exist, WindowList.qml's own header
    // names the fallback (the hyprctl-snapshot technique AltTab/AltTab.qml
    // already uses).
    readonly property var toplevels: Hyprland.toplevels

    // Dispatch over Hyprland's own IPC socket — no `hyprctl` subprocess. Kept
    // as a general passthrough; the workspace strip switches through the
    // model's own `activate()` instead and doesn't need this. This build's Lua
    // config repurposes the socket's `dispatch` command to EVALUATE its
    // argument as Lua — the traditional two-token form
    // ("togglespecialworkspace scratch") fails with "hl.dispatch: expected a
    // dispatcher". Confirmed by sending the raw request directly over the IPC
    // socket, bypassing `hyprctl` and this function, so it's Hyprland itself
    // doing this. Every `request` here needs the Lua-call form instead, e.g.
    // 'hl.dsp.workspace.toggle_special("scratch")' see
    // Bar/modules/Workspaces.qml's scratchpad button for a working example.
