...
// `_findWiredDevice()` below is that same loop, filtered on `DeviceType.Wired`
// instead of `DeviceType.Wifi`.
//
// Deliberately no scan/connect surface here, unlike WifiBridge: a wired link
// has no network to pick — it's plugged in or it isn't — so the whole
// "available networks" half of WifiBridge.qml doesn't apply.

Singleton {
    id: root

    readonly property NetworkDevice device: _findWiredDevice()
    // Hardware presence, not link state — a bar module reads this to hide the
    // icon entirely on a host with no wired NIC at all, rather than showing a
    // permanent "off": a wired port either physically exists or it doesn't.
