...
PanelWindow {
    ...
    Widgets.Scrim {
        anchors.fill: parent
        shown: root.shown
        // A destructive confirmation (reboot, forget a VPN config, …) is
        // the same weight of full-attention blocking surface as
        // screenshot/overview/battery-alert, so it gets the same stronger
        // dim.
        strong: true
    }
    ...
    Item {
        ...
        Widgets.Panel {
            ...
            // this card has focus (Cancel is never destructive). Return
            // is deliberately NOT defaulted to Confirm here the way
            // Launcher's own confirm subview defaults it — that view is
            // only ever reached by a user who just Tab-navigated into it;
            // this dialog can pop up from other, less deliberate entry
            // points (Super+M), so an unfocused stray Return should do
            // nothing rather than run a destructive action. Each button's
            // own Keys.onReturnPressed below still confirms/cancels once
            // the user has actually tabbed to it.
