...
// full-screen transparent PanelWindow would intercept pointer input
// across the WHOLE screen (layer-shell surfaces claim their full
// geometry as their input region unless sized down), breaking click-
// through to every window underneath. Sizing the window itself to just
...
// monitor. Flagged for cheap veto, the same standing caveat every prior
// single-vs-per-screen call in this repo carries.
PanelWindow {
    id: root
    ...
    // No margin, deliberately: the TODO names "positioning the mouse in
    // the corner" as the trigger, the same macOS hot-corner gesture that
    // flings the pointer to the true screen edge — an inset tab (Toast's
    // own margins.bottom/right, copied at first) would sit in a gap the
    // gesture overshoots, and never register as hovered at all.

    readonly property real editorWidth: chWidth * 42
    readonly property real editorHeight: chWidth * 26

    // Gated on cardFade.visible (== shown || still fading out), not
    // root.shown directly: shrinking the window the instant `shown` goes
    // false would clip cardFade's own fade-out mid-animation, since the
    // window's geometry and the card's opacity would then be animating
    // in opposite directions at once. Keeping the window editor-sized
    // until the fade has actually finished avoids that.
    ...
    // Widgets/TextField.qml's own header documents). Merged into this one
    ...
    // The corner "tab" — always present, scales up on hover (the "small
    // transition" the TODO names), tap toggles the editor open/closed.
    Item {
        id: tab
        readonly property real restSize: root.chWidth * 1.6
        readonly property real hoverSize: root.chWidth * 3.2
        // No root.shown branch here: `tab` is already invisible whenever
        // shown is true (see `visible` below), so that branch never reads.
    ...
    // Dialogs/BatteryAlert.qml, and the same `shown || opacity > 0`
    // visibility so the fade-OUT actually renders instead of the Panel
    // vanishing the instant `shown` flips false.
    Item {
        id: cardFade
        anchors.fill: parent
        ...
        Widgets.Panel {
            ...
            Column {
                ...
                Widgets.Panel {
                    ...
                    // (its contentItem is already inset from this outer
                    // height) — nothing extra to subtract here beyond the
                    // header and the one Column spacing gap between them.
