...
// describes), a full-width hairline, then the rows stacked flush with no gap
// (each Modules.SettingsRow draws its own top hairline after the first). A
...
// Modules.SettingsRow and assumes its children stack with no gap. A group may
// carry its own `optionId` (a Settings/options.js catalogue entry): it then
// registers with Services/SettingsPanel like a Modules.SettingsRow so
// `reveal("connectivity.bluetooth")` or a search selection scrolls the content
// pane to the whole group and pulses it — for a section (General the package
// lists) whose "options" are groups, not individual rows.
...
Item {
    ...
    // A group that shows rendered samples rather than controls. It gets a
    // marked title and its body sits on a recessed surface, so a preview never
    // reads as another block of settings.
    property bool preview: false
    default property alias content: body.data

    // No settings module ever collapses out of sight just because a capability
    // is missing (user directive — a hidden section reads as "this feature
    // doesn't exist" rather than "this machine doesn't have it"). A
    // capability-gated group sets `disabled` instead of `visible`: the title,
    // caption and rule stay put, `disabledReason` explains why in place of the
    // rows, and the rows themselves stay in the tree dimmed to
    // WidgetStates.INACTIVE_OPACITY, the same "same weight reduced opacity"
    // affordance every disabled control already uses rather than being torn
    // down and losing scroll/search position.
    property bool disabled: false
    property string disabledReason: ""
    // Same "advanced" gate Modules.SettingsRow.qml carries, at whole-group
    // granularity — for a group that is entirely power-user detail rather than
    // one row inside an otherwise-ordinary group. Hidden unless
    // Services.SettingsPanel.showAdvanced, unless a live search already
    // matches the group.
    ...
    // The shade fill separating this group from its neighbours. `surface2`,
    // not `surface1`: `surface1` is already the Settings panel's own
    // background colour, so a same-shade group fill on top of it would be
    // indistinguishable from no fill at all.
    Rectangle {
        anchors.fill: parent
    ...
    Column {
        ...
        Row {
            ...
            // A preview group is tagged so it never reads as another block of
            // settings.
            Rectangle {
                anchors.verticalCenter: parent.verticalCenter
    ...
    Item {
        ...
        // Mirrors Widgets/WidgetStates.js's own INACTIVE_OPACITY duplicated
        // here rather than importing that file, since every existing importer
        // of it is a sibling inside Widgets/ itself and this is the one
        // Settings-panel structural file outside it.
