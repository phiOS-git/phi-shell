...
PanelWindow {
    ...
    // genuine, distinct backspace presses within this window cancel the lock; see
    // searchField's Keys.onPressed for why isAutoRepeat is what actually keeps a
    // HELD key from doing this on its own — this window only bounds how far apart
    // the two real presses may be.
    ...
    // "command" result) and { kind: "confirm", action: "..." } (Enter on a
    // destructive "system" result).
    property var views: []
    ...
    // Full-screen transparent window so a click anywhere outside the runner box
    // can close it. exclusiveZone -1 + Overlay so a click on the bar strip also
    // dismisses the runner and the box sits above the bar. (No scrim on the runner
    // — it stays a light overlay.)
    ...
    // visibleRows/maxListBoxHeight are the cap (up to 20 rows, or 62% of the
    // screen, whichever is smaller) — currentListBoxHeight below shrinks that cap
    // to fit the actual result count. See panelWrap's own comment for how the box
    // stays centred on screen, top-anchored only in the sense that the top edge
    // doesn't move as the box's height changes.
    ...
    // Shrinks to fit resultList's own implicitHeight (n rows, or the "no results"
    // label's height, or 0 when neither is showing — Column excludes invisible
    // children from that sum on its own), capped at maxListBoxHeight so it never
    // grows past that maximum.
    ...
    // panelWrap below to compute a height-independent anchor position, not by
    // panel itself, which always sizes to its own, possibly smaller, actual
    // content.
    readonly property real maxPanelHeight: inputRow.height + layout.spacing
        + root.maxListBoxHeight + panel.padding * 2

    // The input prefix and the pixel width it occupies — the result options are
    // indented to start exactly where the typed text does.
    ...
    // PanelWindow has no `opacity` property — Components/Toast.qml's own note. The
    // fade lives on `fadeRoot` below instead, a plain Item with a real, animatable
    ...
    // directly as the user types (Qt's C++ side, not this file's own binding), and
    // assigning to a bound property breaks that binding the first time it happens
    // — after which "text: root.queryText" would silently stop tracking
    // root.queryText at all, and a later reset here would never reach the field.
    // commandField below has the identical shape and the identical reason.
    function setShown(v) {
        root.shown = v
        // Mirrors into Services/Launcher.qml so Bar/modules/Runner.qml (a different
        // component tree) can bind its own `active` state to whether the runner bar is
        // open.
    ...
    Timer {
        ...
        interval: 600
        onTriggered: {
            // Only if the query is still the same one that loading — if the user kept
            // typing, queryDebounce's own re-query already superseded this, and firing
            // again here would just re-run a stale query a beat after a fresher one.
    ...
    // (Quickshell.DesktopEntries) rather than querying `phi query ""`, which
    // returns nothing by design. A typed query still goes to `phi query` for real
    // ranking.
    ...
    // The `rich` payload of the currently highlighted result, if it has one
    // (calculator steps/roots/plot, a converter's alternate units). Drives the
    // side card in fadeRoot below.
    ...
    property Component queryComponent: Component {
        Process {
            ...
            // The locked keyword at the moment this Process spawned. When set, --prefix
            // restricts phi to that keyword's provider(s), and queryArg (the visible
            // remainder, keyword already stripped by _lockPrefix()) has the keyword put
            // back in front for the actual argv — every routed provider expects to see its
            // own keyword leading the text it strips itself.
    ...
    function activate(result) {
        if (!result) return
        // ActionLoading is not a real result yet — selecting one records nothing and
        // closes nothing, it just sits there until the retry timer above replaces it
        // (or the user keeps typing).
    ...
    // kitty is this project's confirmed default terminal — hardcoded here for lack
    ...
    function _performAction(action) {
        ...
        case "exec":
            ...
            break
        case "desktopEntry": {
            // Browse-mode result — prefer Quickshell's own DesktopEntry.execute() (handles
            // Terminal=, field codes, DBusActivatable); fall back to its cleaned exec
            // string.
        ...
        case "execTerminal":
            ...
            break
        case "activateWindow":
            // The Lua-call dispatch form — this build's Lua config rejects the traditional
        ...
        case "system":
            ...
            // the other four (lock/suspend/ hibernate/logout) run straight away.
            // Services/PowerActions.qml is the one owner of both the actual commands and
            // this needsConfirm policy, shared with the bar popout's "power" section.
    ...
    // Tab "locks" the keyword currently leading the typed text — strips it from
    // the visible field (it becomes the chip instead) and restricts results to
    // that category (queryComponent above puts it back for the actual query).
    function _lockPrefix(key) {
        root.lockedPrefix = key
        root._lastBackspaceAt = 0
        const lead = key + " "
        if (searchField.text.toLowerCase().indexOf(lead) === 0) {
            // Imperative, not also "root.queryText = rest": searchField's own
            // onTextChanged (see its comment) already syncs queryText out the moment text
            // changes here — a second assignment would just be setting the same value
            // again.
            searchField.text = searchField.text.slice(lead.length)
        }
    }

    // Two cancel paths: clicking the chip's "×" calls this directly; searchField's
    // Keys.onPressed calls it only after two genuine backspace presses on an
    // already-empty field (see that handler's own comment).
    ...
    Widgets.Panel {
        id: panel
        // Explicit, not just the Item default of (0, 0): panelWrap can now be taller
        // than panel (it reserves the box's maximum possible height — panelWrap's own
        // comment), and panel's top edge landing on panelWrap's is exactly what keeps
        // the box's top position fixed as it shrinks or grows.
        ...
        // The runner carries more inner padding than a normal panel — the input and
        // the option list breathe away from the frame, left and right especially.
        ...
        Column {
            ...
            Item {
                ...
                // The locked keyword's chip — the same filled-rounded- rect shape the result
                // list's own selection highlight uses, coloured per-prefix instead of the
                // generic selectionBackground, plus a "×" to remove it.
                Item {
                    id: prefixChip
                ...
                // Same muted-till-hovered "×" grammar Widgets/TextField's own clear button
                // uses, kept as a separate glyph here rather than migrating this field to
                // TextField — this input's arrow-key/Tab/Escape wiring above is load- bearing
                // launcher behaviour TextField doesn't forward.
                Widgets.StyledIcon {
                    id: clearGlyph
                ...
                TextInput {
                    ...
                    // One-way sync out only (see setShown's own comment): this field's initial
                    // text is empty and stays that way until the user types, so no incoming
                    // binding is needed at all — root.queryText always just follows whatever the
                    // user has actually typed. _lockPrefix() below writes it imperatively for the
                    // one case that needs to change it from outside the field.
                    ...
                    // Cancelling the lock needs a double backspace press, to avoid removing it
                    // while holding backspace down. event.isAutoRepeat is what actually satisfies
                    // "holding down" — Qt's own mechanism for telling a held key's synthetic
                    // repeat stream apart from a genuine second press, which a press-timestamp
                    // window alone can't do (a held key's repeats land inside any window short
                    // enough to still feel like a deliberate double-tap). Only armed when the
                    // field is already empty: backspace still deletes normally otherwise.
            ...
            // Shrinks to fit the current result count (root.currentListBoxHeight), capped
            // at the ~20-row maximum.
            Flickable {
                id: resultFlick
                width: parent.width
                ...
                Column {
                    ...
                    Repeater {
                        ...
                        delegate: Item {
                            ...
                            // The name and the directory sit at opposite ends of the row — the space
                            // between them is maxed out, not a fixed gap after the name. Anchored from the
                            // name's right edge to the panel's right padding and right-aligned, so a path
                            // hugs the right edge (tail visible, elided from the left) however short the
                            // name.
                            Widgets.StyledText {
                                id: dirText
                            ...
                            // Hovering moves the keyboard highlight to match, the conventional behaviour
                    ...
                    Widgets.StyledText {
                        ...
                        kind: "label"
                        text: "no results"
                        // A locked prefix with nothing typed yet still runs no query (root.queryText
                        // is the remainder, empty) — without lockedPrefix here too, that state would
                        // show a coloured chip and border over a blank list with no explanation at
                        // all.
            ...
            // Level 1: two sub-view shapes. "command" — an editable command line reached
            // by Tab on a "command" result. "confirm" — reached by Enter on a destructive
            // "system" result (root.pushConfirmView). Both live in the same Panel/Column
            // so only one height calc is needed; each block's own `visible` (keyed off
            // root.currentView.kind) is what a Column positioner already excludes from
            // `implicitHeight` when false.
            Widgets.Panel {
                width: parent.width
                ...
                Column {
                    ...
                    Column {
                        ...
                        // No incoming "text: ..." binding here either (see setShown's comment):
                        // pushCommandView() sets this field's text imperatively at the moment a
                        // sub-view opens, which is the one time it needs to change from outside the
                        // field itself.
                        TextInput {
                            id: commandField
                            width: parent.width
                    ...
                    Column {
                        ...
                        Row {
                            ...
                            Widgets.StyledButton {
                                ...
                                // Explicit per-button Return/Escape: StyledButton has no keyboard handling of
                                // its own, so without this, tabbing to Cancel and pressing Return would fall
                                // through to confirmSubview's own onReturnPressed below and still perform the
                                // destructive action.
    ...
    // separate overlay rather than a new override property on Widgets.Panel itself
    // — Panel's border colour is entirely computed from its own hover/active/focus
    // state machine, shared by every consumer in the shell; adding an
    // arbitrary-colour override there would be a shared-component change this one
    // feature doesn't need, when a same-geometry sibling drawn on top does the
    // same job with no risk to any other Panel user.
    Rectangle {
        anchors.fill: panel
        radius: panel.radius
        color: "transparent"
    ...
    Item {
        ...
        // richWrap is a sibling of panelWrap, not of panel (a grandchild of panelWrap)
        // — QML only allows anchoring to a parent or a sibling, so the anchor itself
        // has to stay panelWrap.top; panel's actual, possibly-smaller-than-reserved
        // height is folded into the margin instead, since panelWrap now reserves the
        // box's maximum possible height for positioning (see its own comment), which
