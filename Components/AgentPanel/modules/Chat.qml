...
Item {
    ...
    // Out-of-plan: the "Agent offline" state one sentence covering five different real causes.
    // AgentInfra already polls every one of those facts for the Settings section — reuse it — panel says which one is actually true instead of leaving the user to guess.
    function _unit(name) {
    ...
    // the Escape task — Widgets/TextField.qml's own `escaped()` for the general shape;
    // `field` is a raw TextInput (not that widget) so it re-implements the same blur-then-signal locally.
    signal blurred()
    ...
    // Services/Agent.qml's setChatTitle(id, title) fully built, never-called capability — no rename control existed anywhere in this panel.
    ...
    // Mirror the transcript into the project folder when a turn finishes, — dashboard and `phi agent search` see it (D-05).
    Connections {
        target: agent
        function onProcessingChanged() { if (!agent.processing) agent.syncCurrentTranscript() }
        // doSend() clears the composer the instant send() is called, before it's known whether a lazily-created session actually succeeded — restore the exact text if it didn't, instead of it just vanishing with only an error line in the transcript to explain why.
        function onSendFailed(text) { field.text = text }
    }

    // `raw: true` returns the actual stored title;
    // the default reformats opencode's own raw-ISO-timestamp default title for display comment has the full reasoning).
    ...
    Column {
        ...
        Widgets.Panel {
            width: parent.width
            // panels-ux-rework: these agent-panel cards had no height at all — the frame collapsed to a hairline and the content spilled out of it.
            // Height now tracks the content like every other Widgets.Panel in the shell.
            height: offlineCol.implicitHeight + padding * 2
            Column {
                id: offlineCol
                width: parent.width
                spacing: root.chWidth * Config.Appearance.space1
                // Out-of-plan: one static sentence regardless of which of several real causes applied — root.offlineDiagnosis().
                Repeater {
    ...
    Item {
        ...
        Column {
            ...
            Row {
                ...
                // Services/Agent.qml's own setChatTitle(id, title) had no UI path to it anywhere in this panel at all.
                // Only offered once a real session exists nothing to rename in the "new chat" state.
                // A plain SmallButton, not wrapped: Row already skips an invisible child when laying out, and `renameBtn.width` reads the same either way visibility does not zero a Item's width.
                Widgets.SmallButton {
                    id: renameBtn
                    anchors.verticalCenter: parent.verticalCenter
                    ...
                    onClicked: {
                        ...
                        // The RAW title, not the reformatted display string — pre-filling the synthetic "New chat · 14 Sep, 15:27" text would let it get saved back as a real, permanent title the next time this is committed.
                        // Still opencode's own raw-timestamp default at this point, so start the field empty instead prompting a real title rather than proposing a bad one.
                ...
                // Full chat-panel rework : the header's own "Settings" button is gone — Panels/AgentPanel.qml's nav rail already grew a Settings icon reachable from every section, so this second way to reach the identical destination, always visible on screen at the same time as the rail's own icon.
                // "New" demoted to a SmallButton: Panels/tabs/agent/ ChatShell.qml's sidebar now has its own, more prominent "New chat" button as the PRIMARY way to start one — this is a quiet secondary convenience for "start fresh without moving to the sidebar", not the main action.
                Widgets.SmallButton {
                    id: newBtn; label: "New"
                    // Same leaveProject()-first guard as ChatShell.qml's own "New chat" button this is a second way to reach the same action, so it needs the same fix or the project stays silently active whichever button is clicked.
            ...
            Widgets.Panel {
                ...
                Row {
                    id: switchRow
                    spacing: root.chWidth * Config.Appearance.space1
                    // agent.switchTarget is the DESTINATION of an in-flight switch, not agent.activeProject — that still holds the OLD value until the switch lands which would show "new project" even while leaving one.
        ...
        Column {
            ...
            Widgets.Panel {
                ...
                Row {
                    ...
                    // a plain `TextInput` cannot wrap or hold a second line at all — a real limitation for anything longer than one short sentence, and out of step with every mainstream chat composer's own Enter-sends / Shift+Enter-newline convention.
                    // `TextEdit` grows with its content instead of clipping or forcing one line.
                    Flickable {
                        id: fieldScroll
                        ...
                        TextEdit {
                            ...
                            selectByMouse: true
                            // Enter sends;
                            // Shift+Enter inserts a real newline — TextEdit's own default behaviour for a bare Enter so only the un-modified case needs intercepting.
                    ...
                    Widgets.StyledButton {
                        ...
                        anchors.verticalCenter: parent.verticalCenter
                        label: "Send"
                        // Matches doSend()'s own guard — a spinner instead of a button that visually invites a click doing nothing while a turn is already in flight.
        ...
        Flickable {
            ...
            clip: true
            // the previous version force-scrolled to the bottom on EVERY content-height change, unconditionally — scrolling up to reread earlier history got yanked straight back down the instant the next streamed token/message grew the transcript, the exact "stop stealing my scroll position" complaint every real chat app (Slack, Discord, ChatGPT) has already had to fix.
            // Now only autoscrolls if the user already at (or within ~2 lines of) the bottom BEFORE this change — `_prevContentHeight` is the previous height, captured deliberately instead of comparing against `contentHeight`'s own new value which would always read "not at the bottom" right after growing.
            ...
            Column {
                id: messages
                width: history.width
                // Each bubble now carries its own role label, — rows need less air between them than the old label-less stack.
                spacing: root.chWidth * Config.Appearance.space2
        ...
        // Persona/personality picker popover.
        // Positioned in `main`'s own coordinate space rather than a Quickshell PopupWindow — no cross-window anchor- direction risk to get wrong.
        Widgets.Panel {
            id: personaCard
            ...
            Column {
                ...
                // Full chat-panel rework : this used to route to the separate "dashboard" destination — user could click into a project to edit its personalities — that destination exists, so there is nowhere left to "navigate" to and this button is gone.
                // Editing a personality is just clicking the project in the sidebar that's already on screen.
            }
        }

        // Click-outside-closes for the popover — same shape Dialogs/PowerMenu.qml's own fadeRoot MouseArea already uses.
        // Below the popover in paint order but everything else in `main`, and only intercepts clicks while actually open.
        MouseArea {
            anchors.fill: parent
            visible: root.personaOpen
            z: 9
    ...
    // this the field UNCONDITIONALLY after calling agent.send() but Services.Agent.send() itself no-ops while a turn is already in flight (`if (sendProc.running...) return`, by design, correctly preventing a real double-send race at the backend).
    // The UI side of that guard missing entirely: pressing Enter/Send while waiting for a reply silently ERASED whatever typed, with nothing actually sent — real, silent data loss, not just a missing loading indicator.
    // Now a no-op the same way the backend already is: nothing is cleared, nothing is lost, and the message is still sitting there ready to send the moment the turn finishes.
    function doSend() {
