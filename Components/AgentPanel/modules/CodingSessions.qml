...
// The agent panel's CodingSessions section. A2 sessions, active and past, from
// phi-owned metadata files — the panel never reaches A2's server. Per session:
// open the mirrored transcript (read-only), focus the terminal window, or open
// a fresh one.
...
Item {
    ...
    // Keys.onEscapePressed for the full reasoning. Having a transcript open is
    // this tab's own "one level deeper" state.
    ...
    // support services weren't running, phi-agent-contain's socat bridge never
    // found its socket and the failure happened invisibly inside that terminal
    // window. Reuse the same unit facts AgentInfra already polls for Settings
    // to warn here BEFORE a session is opened, instead of after it silently
    // fails.
