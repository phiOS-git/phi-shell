...
Item {
    ...
    // `glyph` is a plain font symbol and cannot express a custom animated icon. A
    // caller needing one sets `iconDelegate` instead and leaves `glyph` empty —
    // see the `customIcon` Loader below for sizing.
    property Component iconDelegate: null
    // The label-side mirror of `iconDelegate`: a caller needing rich label content
    // sets `labelDelegate` instead of (or alongside, when labelFirst) `label`. The
    // loaded item positions where the StyledText label would.
    ...
    // "workspace" is a bar-button ambient too — same mono font, padding and hover
    // sweep as "isle" — differing only in the colour recipe and in `contentColor`
    // below, whose active state must show `stateColors.fg` rather than the
    // accent-text override other isle buttons use. Conditions that really ask "is
    // this a bar button" read `root._bar`, so a third bar ambient needs one line
    // here rather than a hunt through five conditionals.
    ...
    // "wider when selected", so a caller that wants one drives this. 0 elsewhere.
    ...
    // Screen x of this button's RIGHT edge, so a popout can hang directly under it
    // instead of in the corner. Guarded: mapToItem(null) can throw before the item
    // is in a scene, and callers treat 0 as "fall back to a corner".
    function rightX() {
        try {
    ...
    // The LEFT edge, same idea. Near the screen's left edge, right-edge alignment
    // would pin a popout's far side there and push most of the card off-screen, so
    // a left-isle consumer anchors to this instead.
    function leftX() {
        try {
    ...
    // Label/icon colour. invalid wins outright, then a plain active segment wins
    // over `tone`: active draws no fill or border of its own, so letting tone win
    // would leave a selected-but-toned button with nothing marking it selected.
    //
    // Two exclusions from the accent-active override: `accentWhenActive` already
    // gets its accent through `stateColors.fg`, and the "workspace" ambient uses
    // inverted colours rather than accent text, so its `stateColors.fg` has to
    // reach the label.
    ...
    // Floor the content box against the mono cell height: the symbol font's glyph
    // box is shorter than a text line, so glyph-only buttons came out visibly short
    // beside text ones. Flooring makes every Segment in an isle the same height.
    ...
    // height, so a single digit or glyph reads as a square tile, not a wide pill.
    ...
    // Isle-only hover sweep, top-to-bottom, height anchored to the top. A panel
    // Segment keeps its flat hover fill.
    //
    // Hover-only, deliberately not shared with "active": a shared sweep makes the
    // two indistinguishable, and un-hovering an active button reads as the
    // highlight wrongly sticking. Active uses no fill at all, so the two states
    // share no mechanism and cannot race.
    //
    // Driven off `resolvedState`, not raw `hovered`: resolve() already picks one
    // state by precedence, so a button both focused and hovered shows its focus
    // ring instead of two effects fighting for the same space.
    //
    // Deliberately not a masked reveal of the icon/label content: several bar icons
    // are procedural Canvas drawings with infinite animations, and rendering each
    // `iconDelegate` a second time to clip it would double that cost for a hover
    // micro-interaction. Instead this Rectangle sweeps the background while
    // `contentColor` fades via the Behaviors StyledText and StyledIcon already
    // carry. A Canvas `iconDelegate` has no such Behavior, so those icons snap
    // colour rather than fade.
    //
    // The condition is inlined in the handler rather than read from a separate
    // `_sweepOn` property: both would depend on the same `resolvedState` change and
    // QML does not order two dependents of one source, so the handler could read a
    // stale value and set hoverAmount to 1 exactly as the mouse left.
    ...
    Item {
        // A plain Item, not a Row: Qt's Row docs forbid a child anchoring itself
        // horizontally, and icon and text anchor to each other. Explicit rounded x/y
        // rather than `anchors.centerIn`, which computes (parent - child) / 2 and goes
        // fractional on an odd difference — a real, visible gap. Math.round pins it to
        // a whole pixel.
        id: layout
        ...
        // The delegate binds its own properties against the enclosing file's Segment id
        // rather than this Loader pushing them in, so a typo is a QML load error rather
        // than a silent no-op.
        Loader {
            id: customIcon
            ...
            // No explicit implicit size binding: Loader already forwards the loaded item's
            // implicit size itself, and a binding here would fight that internal write.
            anchors.left: root.labelFirst && layout._labelShown ? (customLabel.active ? customLabel.right : labelText.right) : parent.left
            anchors.leftMargin: root.labelFirst && layout._labelShown ? root.gap : 0
            anchors.verticalCenter: parent.verticalCenter
        }

        // The label-side mirror of `customIcon`. A caller setting `labelDelegate` owns
        // the whole label slot and binds against the enclosing Segment id.
        Loader {
            id: customLabel
    ...
    TapHandler {
        id: tapHandler
        enabled: root.enabled && !root.loading
        // `ReleaseWithinBounds`, not TapHandler's default `DragThreshold`, which
        // cancels the tap if the pointer moves ~10px between press and release — easy
        // for a finger. Since `pressed` alone drives the full inverted active visual,
        // the default made a touch look like the highlight fired but the click did not.
        // `ReleaseWithinBounds` only cancels when the release lands outside the Item.
        gesturePolicy: TapHandler.ReleaseWithinBounds
        // `margin` widens the release-bounds tolerance for a press that lands and lifts
        // right at the edge. Reused from `paddingV` rather than a new literal. Left off
        // `hoverHandler` on purpose: BarIsle packs Segments with zero spacing, so a
        // wider hover region would overlap adjacent buttons. Not gated on touchscreen
        // capability — a forgiving release tolerance is correct for a mouse too.
