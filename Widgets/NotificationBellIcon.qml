...
Item {
    ...
    // Bar/glyphs.js already draws every bar icon from. Duplicated here rather
    // than importing glyphs.js (a Bar/-scoped file; Widgets/ stays independent
    // of it, matching every sibling icon in this family which take their
    // symbol from the caller instead of reading it themselves).
    ...
    Item {
        ...
        scale: 1.0
        // No `Behavior on scale` here, deliberately: `dndPop` below already
        // drives `scale` with its own explicit two-step NumberAnimation. A
        // Behavior watches every write to the property it's attached to,
        // including ones an unrelated Animation makes directly — stacking one
        // on top of the other would have them fighting over the same property
    ...
    // arrives wrapped in the caller's category-B Behavior, which reassigns it
    // on every animation frame of its ~120ms ramp (same reasoning
    // SunMoonIcon's header gives for why a Behavior-driven property repaints
    // every frame). A restart() on every one of those frames would abort
    // dndPop's two-leg sequence over and over and only ever let it play out
    // after the ramp itself finishes, well behind the crossfade instead of
    // together with it.
    ...
    // Overlapping calls restart rather than queue: a burst of notifications
    // should not stack effects.
