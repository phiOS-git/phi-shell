...
// keeps the native `Rectangle` it always drew, so this costs nothing for
// the common case.
...
Item {
    ...
    // Both default to "no override" so every existing consumer keeps its
    // exact current look. Only applies while `!root.invalid` (see the
    // Rectangle below) — a caller wanting a softer RESTING border still
    // gets the real error colour the instant something actually goes
    // wrong; this never softens the one state that has to stay loud.
    ...
    // (the bar's own background) so the shell reads as a continuation of
    // the bar; any Panel nested inside it (an inner section, e.g.
    // QuickNote's own `textPanel`) leaves this at its default and keeps
    // the ordinary "shaded" surface1/2/3 ramp, which is what actually
    // gives an inner section its own distinct background.
    ...
    // The resolved foreground for this panel's content. A Panel with a
    // plain content slot (a bare StyledText, a Column of them) cannot have
    // its children recoloured from here — QML has no ancestor colour
    // inheritance — so a consumer that shows selectable text inside a
    // Panel binds its text `color` to this instead of hand-rolling a
    // `selected ? selectionText : textPrimary` ternary at each call site.
    // Mirrors Segment.contentColor. `active` does not invert fg to the
    // background colour, so this is ordinary full-contrast ink in every
    ...
    // `color` value and a plain string — it never coerces, so it evaluates
    // to `true` unconditionally, even for an untouched default override
    ...
    // colour components are ordinary numeric comparisons, not a type
    // mismatch.
