...
Column {
    ...
    // A plain QML/QtQuick Item is not dynamically extensible the way a bare JS
    // object is — assigning an undeclared property name onto a QObject-derived
    // instance is a hard error, not a silent add, so the per-child animation
    // objects live in THIS widget's own `_anims` rather than attached to the
    // child itself. `Qt.createQmlObject`'s second argument is `root` not
    // `child` — nothing about owning the animation requires the child to be
    // its QML parent.
    property var _anims: []
    ...
    // QML-idiomatic way to give an arbitrary, caller-supplied Item an animated
    // property. A creation failure just leaves that child static — fails open
    // onto "no stagger", never onto a broken panel. Opacity only, deliberately
    // no position slide: `Column` itself authoritatively assigns every child's
    // `y` on each relayout — a second, independent animation driving `y` would
    // fight that assignment instead of cooperating with it, unlike QtQuick's
    // own built-in positioner `add`/`move` transitions. Fading is the one
    // channel free to animate without that conflict.
    function _animate(child, index) {
        if (root._anims[index] === undefined) {
            try {
                // Plain `NumberAnimation`/`PropertyAnimation` has no `delay`
                // property in QtQuick. A `PauseAnimation` ahead of the real
                // `NumberAnimation` inside a `SequentialAnimation` is
                // QtQuick's actual mechanism for "wait, then animate"
                // `animations` is `SequentialAnimation`'s default list
                // property, so the two children are reachable by index with no need
                // for `id`s inside the dynamically-created string.
