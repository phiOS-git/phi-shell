...
Item {
    ...
    // Not named `color`: legal on an Item, but `color` doubling as both a QML
    // value type and a property name is a nested-scope-resolution footgun
    // worth just avoiding.
    property color iconColor: "white"
    property int sizeStep: 2
    // 0 = full moon (crescent), 1 = full sun (complete disc + rays). Plain
    // external property — wrap it in a Behavior at the call site, this widget
    // only reacts to whatever value arrives.
    ...
    // Segment.qml's Loader, and the inner Canvas fills `parent` — if any link
    // in Loader-forwards-implicit-size chain resolved to 0 for any reason,
    ...
    // Linear in dayness. The two ends are picked — shadow disc clears the
    // OUTERMOST thing drawn at each end, not just the body disc: at dayness=1
    // the rays reach out to R + rayGap + rayLen = 1.647R from centre, so the
    // shadow's NEAR edge (shadowOffsetX - rShadow) needs to clear 1.647R for
    // the rays to render whole, not just the 1.0R the disc alone would need —
    // 2.9R gives that, with margin to spare. At dayness=0 the offset is 0.55R:
    // heavy overlap, a crescent left on the far side.
    readonly property real _shadowOffsetX: _r * (0.55 + 2.35 * root.dayness)

    // `_boxSize` derives from `sizeStep` and Config.Appearance's font tokens —
    // the font-size scale never changes after this item is created, so no
    // handler is needed for it. A theme variant switch does not restart the
    // shell, but font metrics are a separate, structural token family a
    // colour-variant switch never touches.
    ...
    Canvas {
        ...
        onPaint: {
            ...
            // 1. rays — length and opacity both track dayness, drawn first —
            // shadow cutout can eclipse the ones on its side.
            ...
            // 2. body disc — a dim "track" for the full disc, always present,
